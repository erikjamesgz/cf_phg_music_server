...
function rstr2hex(input: string): string {
  ...
  let output = "";
  for (let i = 0; i < input.length; i++) { const x = input.charCodeAt(i); output += hexChars.charAt((x >>> 4) & 0x0f) + hexChars.charAt(x & 0x0f); }
  return output;
}

export function md5(input: string): string { return rstr2hex(rstrMD5(input)); }
